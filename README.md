...
Добавлен новый модуль, для взаимодейтвия с БД.
  
**ВАЖНО**:
Для запуска у вас должна быть создана БД с таким же именем как у меня ИЛИ можно самостоятельно настроить в файле application.properties
В файлах init.sql и initDB.txt находяться изначальные данные для заполнения c помощью которых я проверял данные. 


В проекте есть 2 ендпоинта: 
http://localhost:8081/books/popular - возращает топ3 популярные книги среди детей ДО 10 лет включительно.

http://localhost:8081/books?userId= - возращает книги, который брал пользователь по id пользователя..

Также, данные пользователю возращаются не с БД напрямую, а с DTO.

</details>


## 4 часть задания
Добавлены тесты для 2 и 3 сервиса.
## 5 часть задания
В модуль с БД databaseproject
Добавлен spring cache, который реализован на redis, который в свою очередь реализован через redisson.
